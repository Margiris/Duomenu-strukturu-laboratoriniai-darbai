...
import Lab4Burakauskas.MapBURx;
import Lab4Burakauskas.Smartphone;
...
import Lab4Burakauskas.Production;
import Lab4Burakauskas.SpeedTest;
...
public class Lab4Window extends JFrame implements ActionListener {
    ...
    private MapADTx<String, Smartphone> map;
    ...
    private void initComponents() {
        ...
        panButtons = new Panels(new String[]{
                ...
                MESSAGES.getString("button8")}, 1, NUMBER_OF_BUTTONS);
        panButtons.getButtons().forEach((btn) -> btn.addActionListener(this));
        IntStream.of(1, 3, 4, 5, 6, 7).forEach(p -> panButtons.getButtons().get(p).setEnabled(false));
...
//                taInput,
        ...
        Stream.of(panParam1, panParam2, panParam3).forEach(panParam123::add);
    ...
    private void appearance() {
        ...
        List<JTextField> fields = panParam3.getTfOfTable();
        IntStream.of(1, 2, 3).forEach(p ->
                fields.get(p).setInputVerifier(new InputVerifier() {
                    @Override
                    public boolean verify(JComponent input) {
        ...
        fields.get(4).setInputVerifier(new InputVerifier() {
            @Override
            public boolean verify(JComponent input) {
                String text = ((JTextComponent) input).getText();
                try {
        ...
        fields.forEach(comp -> comp.addFocusListener(new FocusListener() {
            @Override
        ...
        taEvents.setWrapStyleWord(true);
    }
    ...
    public void containsValue() {
        Smartphone smartphone = readObjectData();
        if (map.containsValue(smartphone))
            KsSwing.oun(taEvents, smartphone != null ? smartphone.toString() : null, MESSAGES.getString("msg7"));
        else
            KsSwing.oun(taEvents, smartphone != null ? smartphone.toString() : null, MESSAGES.getString("msg8"));
    }

    public void keySet() {
        Set<String> set = map.keySet();
        KsSwing.oun(taEvents, set.toString(), MESSAGES.getString("msg9"));
    }

    public void averageChainLength() {
        float average = map.averageChainLength();
        KsSwing.oun(taEvents, average, MESSAGES.getString("msg10"));
    }

    public void numberOfEmpties() {
        KsSwing.oun(taEvents, map.numberOfEmpties(), MESSAGES.getString("msg13"));
    }

    public void putIfAbsent() {
        Smartphone smartphone = readObjectData();
        int key = Math.abs(smartphone != null ? smartphone.hashCode() : 0);
        key = key / 10000;
        smartphone = map.putIfAbsent(Production.ID_CODE + key, smartphone);
        ...
        if (smartphone == null)
            KsSwing.oun(taEvents, Production.ID_CODE + key, MESSAGES.getString("msg11"));
        else
            KsSwing.oun(taEvents, smartphone.toString(), String.format(MESSAGES.getString("msg12"), Production.ID_CODE + key));
    }
    ...
    public void mapGeneration(String filePath) {
        ...
        createMap();
        // Jei failas nenurodytas - generuojami telefonai ir talpinami atvaizdyje
        if (filePath == null) {
            Smartphone[] smartphonesArray = production.produceAndSellSmartphones(sizeOfGenSet, sizeOfInitialSubSet);
            for (Smartphone a : smartphonesArray) {
                map.put(
                        production.getFromBaseSmartphoneID(),//raktas
                        a);
            }
        ...
        IntStream.of(1, 3, 4, 5, 6, 7).forEach(p -> panButtons.getButtons().get(p).setEnabled(true));
    }

    public void mapAdd() throws MyException {
        Smartphone a = production.sellSmartphone();
        map.put(
                production.getFromBaseSmartphoneID(),// Raktas
                a);
        table.setModel(new TableModel(map.getModelList(panParam1.getTfOfTable().get(5).getText()),
    ...
    private Smartphone readObjectData() {
        List<JTextField> fields = panParam3.getTfOfTable();
        ...
        for (JTextField f : fields) {
            data.append(f.getText()).append(" ");
        }
        return new Smartphone(data.toString());
    }
    ...
    private void createMap() {
        switch (cmbCollisionTypes.getSelectedIndex()) {
            case 0:
                map = new MapKTUx<>("", new Smartphone(), initialCapacity, loadFactor, ht);
                break;
            // ...
            case 3:
                map = new MapBURx<>("", new Smartphone(), initialCapacity, loadFactor, ht);
                break;
