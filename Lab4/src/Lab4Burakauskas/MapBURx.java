...
@SuppressWarnings({"unchecked", "WeakerAccess", "unused", "JavaDoc", "Duplicates"})
public class MapBURx<K, V> extends MapBUR<K, V> implements MapADTx<K, V> {
     ...
     * @param baseObj
     */
    public MapBURx(K baseKey, V baseObj) {
     ...
     * @param ht
     */
    public MapBURx(K baseKey, V baseObj, HashType ht) {
     ...
     * @param ht
     */
    public MapBURx(K baseKey, V baseObj, int initialCapacity, HashType ht) {
     ...
     * @param ht
     */
    public MapBURx(K baseKey, V baseObj, int initialCapacity, float loadFactor, HashType ht) {
    ...
    public void load(String filePath) {
        ...
        if ((baseKey == null) || (baseObj == null)) { // Elementų kūrimui reikalingas baseObj
            Ks.ern("Naudojant load-metodą, "
                    + "reikia taikyti konstruktorių = new MapBUR(new Data(),new Data())");
        }
        try {
            try (BufferedReader fReader = Files.newBufferedReader(Paths.get(filePath), Charset.defaultCharset())) {
                fReader.lines().filter(line -> line != null && !line.isEmpty()).forEach(this::put);
            }
